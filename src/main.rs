...
fn main() {
    ...
    while let Some(arg) = args.next() {
        match &*arg {
            ...
            x if x.starts_with("--") => {
                eprintln!(
                    "Unknown option {}. Run {} --help for list of options",
                    x, prg_name
                );
                std::process::exit(1);
            }
            x if x.starts_with('-') => {
                for c in x[1..].chars() {
                    match c {
                        ...
                        x => {
                            eprintln!(
                                "Unknown short option -{}. Run {} --help for list of options",
                                x, prg_name
                            );
                        }
                    }
    ...
    let mut input_file = unwrap_or_error!(
        input_file.canonicalize(),
        "Cannot find path containing {}: {error}",
        input_file.display()
    );
    input_file.pop();
    ...
    unwrap_or_error!(
        expander::find_modules(&mut file, &mut search),
        "Cannot open {} for reading: {error}",
        search.error_search().display()
    );
